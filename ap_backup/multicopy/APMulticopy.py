...
parser.add_option("--num-copies", dest="num_copies", default=5, type="int",
                  ...
                  metavar="NUMBER")
parser.add_option("--min-period-days", dest="min_period_days", default=0, type="int",
                  help="minimum number of days since last backup; 0 to force copy in any case; default is 0",
                  metavar="NUMBER")
parser.add_option("--target-base-name", dest="target_base_name", default=None,
...
#get parameters
src_file_or_folder = args[0]
target_dir = args[1]

#run
apbackup.multicopy.multiCopy(
    src_file_or_folder,
    target_dir,
    num_copies=options.num_copies,
    target_base_name=options.target_base_name,
    min_period_days=options.min_period_days,
    append_time=options.append_time,
